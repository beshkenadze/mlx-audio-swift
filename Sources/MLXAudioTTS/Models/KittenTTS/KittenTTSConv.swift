...
private func weightNorm(_ v: MLXArray, _ g: MLXArray, dim: Int = 0) -> MLXArray {
    let rank = v.ndim
...
class KittenConvWeighted: Module {
    let stride: Int
    let padding: Int
    let dilation: Int
    let groups: Int
    let encode: Bool
    // Property names must match weight keys: weight_g, weight_v, bias
    ...
    func callAsFunction(_ x: MLXArray, op: ConvOp = .conv1d) -> MLXArray {
