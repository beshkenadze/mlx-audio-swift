...
class KittenAlbertEmbeddings: Module {
    ...
    init(config: KittenTTSPLBertConfig, vocabSize: Int) {
...
class KittenAlbertSelfAttention: Module {
    ...
    init(config: KittenTTSPLBertConfig) {
...
class KittenAlbertLayer: Module {
    @ModuleInfo var attention: KittenAlbertSelfAttention
    ...
    init(config: KittenTTSPLBertConfig) {
        _attention = ModuleInfo(wrappedValue: KittenAlbertSelfAttention(config: config))
...
class KittenAlbertLayerGroup: Module {
    @ModuleInfo(key: "albert_layers") var albertLayers: [KittenAlbertLayer]

    init(config: KittenTTSPLBertConfig) {
        _albertLayers = ModuleInfo(wrappedValue: (0..<config.innerGroupNum).map { _ in KittenAlbertLayer(config: config) }, key: "albert_layers")
    }
...
class KittenAlbertEncoder: Module {
    let config: KittenTTSPLBertConfig
    @ModuleInfo(key: "embedding_hidden_mapping_in") var embeddingMapping: Linear
    @ModuleInfo(key: "albert_layer_groups") var albertLayerGroups: [KittenAlbertLayerGroup]

    init(config: KittenTTSPLBertConfig) {
        self.config = config
        _embeddingMapping = ModuleInfo(wrappedValue: Linear(config.embeddingSize, config.hiddenSize), key: "embedding_hidden_mapping_in")
        _albertLayerGroups = ModuleInfo(wrappedValue: (0..<config.numHiddenGroups).map { _ in KittenAlbertLayerGroup(config: config) }, key: "albert_layer_groups")
    }
...
class KittenAlbert: Module {
    let config: KittenTTSPLBertConfig
    @ModuleInfo var embeddings: KittenAlbertEmbeddings
    @ModuleInfo var encoder: KittenAlbertEncoder
    @ModuleInfo var pooler: Linear

    init(config: KittenTTSPLBertConfig, vocabSize: Int) {
        self.config = config
        _embeddings = ModuleInfo(wrappedValue: KittenAlbertEmbeddings(config: config, vocabSize: vocabSize))
        _encoder = ModuleInfo(wrappedValue: KittenAlbertEncoder(config: config))
